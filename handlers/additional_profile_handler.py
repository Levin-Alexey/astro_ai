...
async def start_additional_profile_creation(callback: CallbackQuery, state: FSMContext):
    ...
    await message.answer(
        "О, это интересно! 😻\n"
        "Давай заполним анкету для нового человека!\n\n"
        "<b>Как его зовут?</b> 👇🏼",
        parse_mode="HTML",
        reply_markup=kb
    )
...
async def handle_additional_name(message: Message, state: FSMContext):
    ...
    if not name:
        ...
        await message.answer("Пожалуйста, напиши имя текстом ✍️", reply_markup=kb)
        return
...
async def handle_additional_profile_cancel(callback: CallbackQuery, state: FSMContext):
    ...
    # Показываем сообщение об отмене с новой кнопкой
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
    ...
    text = (
        "Хорошо, <b>отменила создание анкеты для нового человека</b> 😼\n"
        "Если снова захочешь добавить новый профиль, ищи кнопку «Начать разбор по новой дате» в Главном меню (/start) или Личном кабинете (/lk) 🤝"
    )
    try:
        await callback.message.edit_text(
            text,
