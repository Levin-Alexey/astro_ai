    ...
    async with get_session() as session:
        ...
        if active_subscription:
            ...
            await callback.message.answer(
                ...
                "▫️ Точный разбор транзитов на сегодня: что включено в небесах и как это может отыграться именно у тебя\n"
                "▫️ Персональные рекомендации: что лучше сделать, а что лучше отложить; куда направить энергию, чтобы день был в плюс, а не «ну да, опять всё не так»\n"
                "— И все это по твоим личным данным (дата, время и место рождения), а не просто по знаку зодиака ☺️ \n\n"
                "💵 Подписка на месяц — всего 99₽ (вместо 5555₽)\n"
                "❗️Без автопродления\n\n"
                "Оплатить по кнопке ниже 👇🏼",
