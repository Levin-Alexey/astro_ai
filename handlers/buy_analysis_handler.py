...
async def handle_buy_for_profile(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        async with get_session() as session:
            ...
            # [DISABLED] Отключена покупка всех планет для доп. профиля
            # Причина: скрываем кнопку, сценарий пока не поддержан
            # Важно: убираем блок, а не callback_data
            # Иначе получится кнопка без действия
