...
Как грамотно и эффективно планировать год именно тебе?
Пиши не как нейтральный психолог, а как близкий друг, который видит суть и не боится говорить честно. Стиль — живой, человеческий, прямой. Ты можешь быть и жёстким, если это поможет человеку очнуться. Но без морали. Не используй заголовки и пункты, пиши одним цельным текстом. Твоя цель — создать ощущение полного узнавания. Используй разговорный стиль без канцеляризмов. Опиши, как это проявляется в повседневной жизни: в реакциях, привычках, мелочах. Пиши напрямую, без приветствий и обращений по имени. Сделай разбор строго до 2500 символов, не больше.
    ...
    try:
        ...
        async with get_session() as session:
            ...
            if not openrouter_client or not OPENROUTER_API_KEY:
                ...
                analysis_content = f"""☿️ Твой персональный разбор Меркурия
...
Это тестовый разбор. После оплаты ты получишь персональный анализ на основе точных астрологических данных!"""
            ...
            if llm_result["success"]:
                ...
                await send_mercury_analysis_to_user(user.telegram_id, error_message, prediction.profile_id)
                return False
